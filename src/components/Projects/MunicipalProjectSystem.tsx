...
// Configuration
// -----------------------------------------------------------------------------
// Placeholders for API keys. Replace these strings with real keys provided by
// your Google Cloud project. The MAPS_API_KEY is reused from the existing
// configuration for map rendering. TRAFFIC_API_KEY is used for Directions
// requests and GEMINI_API_KEY authorizes calls to the generative AI service.
const MAPS_API_KEY = 'AIzaSyBZwtfECQcocEuWwXKrzrn-VkwRe_zOPgc';
const TRAFFIC_API_KEY = 'AIzaSyAxhz4nBPt2wenl86Rhrp5sKVROOiuCv4E';
const GEMINI_API_KEY = 'AIzaSyD6x7cS97BIwWh0dIYJ_Zck6Adj22PuBMk';

// Davis, CA coordinates. Google Maps will center on this location by default.
const DAVIS_CENTER: Coordinate = { lat: 38.544907, lng: -121.740517 };
...
function parsePolygonGeometry(geom: any): { lat: number; lng: number }[] | null {
  if (geom && typeof geom === 'object' && geom.type === 'Polygon' && Array.isArray(geom.coordinates)) {
    return geom.coordinates[0].map((coord: number[]) => ({ lat: coord[1], lng: coord[0] }));
  }
...
// Simple location overlap checker. For points, this function computes the
// great‑circle distance between two coordinates and checks if they fall within
// 100 m of each other. When provided with polygons, the function falls back to
// false. It can be extended to handle polygon intersection if needed.
function checkLocationOverlap(loc1: any, loc2: any): boolean {
  const point1 = parseGeometry(loc1);
  const point2 = parseGeometry(loc2);
  // Only evaluate proximity for point data.
  if (point1 && point2) {
...
interface ProjectCreationModalProps {
  ...
  onClose: () => void;
  onSave: (
    project: Omit<MunicipalProject, 'id' | 'created_at'>,
    location?: Coordinate,
...
const ProjectCreationModal: React.FC<ProjectCreationModalProps> = ({
  ...
  // Check for schedule and location conflicts whenever relevant fields change.
  const checkForConflicts = useCallback(() => {
    ...
    const conflictingProjects = existingProjects.filter(project => {
      ...
      // Determine geometry for the new project (point or polygon).
      const currentLocation = location
        ? { type: 'Point', coordinates: [location.lng, location.lat] }
        : coordinates.length > 0
        ? { type: 'Polygon', coordinates: [coordinates.map(c => [c.lng, c.lat])] }
        : null;
      if (!currentLocation) return false;
      return project.locations.some(loc => checkLocationOverlap(currentLocation, loc.location));
    });
          ...
          <div>
            ...
            <div className="flex gap-2 mb-3">
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 text-sm rounded ${
                  locationType === 'point' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'
                }`}
              >
                <MapPin size={16} />
                Point Location
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 text-sm rounded ${
                  locationType === 'draw' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'
                }`}
              >
                <PenTool size={16} />
                Draw Area
            ...
            <div className="h-64 border border-gray-300 rounded">
              ...
              >
                ...
                {location && locationType === 'point' && <Marker position={location} />}
                {coordinates.length > 0 && locationType === 'draw' && (
                  <Polygon
                    paths={coordinates}
                    options={{ fillColor: '#3b82f6', fillOpacity: 0.3, strokeWeight: 2 }}
                  />
                )}
              </GoogleMap>
            </div>
...
class MunicipalProjectService {
  // Seed data for demonstration. The original project coordinates around
  // Pasadena have been replaced with coordinates in the city of Davis, CA. Each
  // project includes at least one location so markers appear on the map.
  private static projects: ProjectWithLocations[] = [
    {
      id: '1',
      name: 'Downtown Infrastructure Upgrade',
      description: 'Major infrastructure improvements in downtown Davis',
      status: 'in_progress',
      ...
      locations: [
        {
          id: 'loc-1',
          project_id: '1',
          location: { type: 'Point', coordinates: [-121.740517, 38.544907] },
          created_at: '2024-01-01T00:00:00Z'
        }
      ]
    },
      ...
      locations: [
        {
          id: 'loc-3',
          project_id: '3',
          location: { type: 'Point', coordinates: [-121.7410, 38.5450] },
          created_at: '2024-01-03T00:00:00Z'
        }
      ]
    }
  ...
  static async updateProject(id: string, updates: Partial<MunicipalProject>): Promise<ProjectWithLocations | null> {
    ...
    this.projects[index] = { ...this.projects[index], ...updates, updated_at: new Date().toISOString() };
  ...
  static findConflictingProjects(): string[] {
    ...
    for (let i = 0; i < this.projects.length; i++) {
      for (let j = i + 1; j < this.projects.length; j++) {
        ...
        if (!project1.start_date || !project1.end_date || !project2.start_date || !project2.end_date) continue;
        const dateOverlap = dateRangesOverlap(project1.start_date, project1.end_date, project2.start_date, project2.end_date);
        if (dateOverlap) {
          const locationOverlap = project1.locations.some(loc1 => project2.locations.some(loc2 => checkLocationOverlap(loc1.location, loc2.location)));
          if (locationOverlap) {
...
const ProjectMapView: React.FC<ProjectMapViewProps> = ({ projects, onProjectClick, selectedProject, conflictingProjects = [] }) => {
  const containerStyle = { width: '100%', height: '600px' };
  const DEFAULT_CENTER = DAVIS_CENTER;
  const [infoWindow, setInfoWindow] = useState<{
    project: ProjectWithLocations;
    position: Coordinate;
  } | null>(null);
  const { isLoaded } = useJsApiLoader({ id: 'google-map-script', googleMapsApiKey: MAPS_API_KEY, libraries: ['places', 'drawing'] });
  ...
  return (
    <GoogleMap
      ...
      zoom={13}
      options={{ mapTypeId: 'roadmap', streetViewControl: false, fullscreenControl: true, zoomControl: true, mapTypeControl: true }}
        ...
        <InfoWindow position={infoWindow.position} onCloseClick={() => setInfoWindow(null)}>
          <div className="max-w-xs">
            ...
            <div className="text-xs text-gray-500">
              <div>
                Status: <span className="capitalize">{infoWindow.project.status.replace('_', ' ')}</span>
              </div>
              <div>
                Priority: <span className="capitalize">{infoWindow.project.priority}</span>
              </div>
...
const ProjectList: React.FC<ProjectListProps> = ({ projects, onProjectSelect, selectedProject, conflictingProjects = [] }) => {
  ...
  return (
    <div className="bg-white shadow-sm border border-gray-200 rounded-lg">
      <div className="px-6 py-4 border-b border-gray-200">
        <h3 className="text-lg font-medium text-gray-900">Municipal Projects ({projects.length})</h3>
      </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {hasConflict && <AlertCircle size={16} className="text-red-500" title="Schedule/Location Conflict" />}
                  </div>
...
// -----------------------------------------------------------------------------
// AI Traffic Prediction Helpers
// -----------------------------------------------------------------------------
// Fetch traffic information using the Google Maps Directions API. Given a
// project's coordinate, this function queries estimated travel times between
// that point and the center of Davis with live traffic considerations. The
// result includes duration_in_traffic for the fastest route. For more advanced
// routing scenarios, you might call the Routes API or add additional
// parameters (mode=driving, departure_time=now, traffic_model=best_guess). See
// Google’s documentation for more details【584297902871315†L304-L324】.
async function fetchTrafficInfo(coordinate: Coordinate): Promise<any> {
  try {
    const origin = `${coordinate.lat},${coordinate.lng}`;
    const destination = `${DAVIS_CENTER.lat},${DAVIS_CENTER.lng}`;
    const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${encodeURIComponent(
      origin
    )}&destination=${encodeURIComponent(destination)}&departure_time=now&traffic_model=best_guess&key=${TRAFFIC_API_KEY}`;
...
// Generate a natural language summary of traffic impact using Gemini. Given a
// project and traffic data, this function crafts a prompt for the Gemini API
// describing the project and the raw traffic data. Gemini uses patterns and
// historical traffic insights to predict future conditions, as described by
// Google’s explanation of their traffic prediction models【584297902871315†L304-L336】. The
// returned summary is truncated to the first candidate.
async function generateTrafficSummary(project: ProjectWithLocations, trafficData: any): Promise<string> {
  try {
    // Compose a descriptive prompt. We include key fields from the project,
    // such as its name, schedule, budget and location, along with the raw
    // traffic response. The AI is instructed to summarise how construction
    // might affect local travel and suggest alternative routes.
    const prompt = `You are an expert transportation planner. Given the following municipal project details and live traffic data, provide a concise summary (3-4 sentences) explaining how this project could impact traffic and suggest potential detour routes.\n\n` +
      `Project name: ${project.name}\n` +
      `Description: ${project.description ?? 'No description'}\n` +
      `Schedule: ${project.start_date ?? 'N/A'} to ${project.end_date ?? 'N/A'}\n` +
      `Location: ${JSON.stringify(project.locations[0].location)}\n` +
      `Traffic API response: ${JSON.stringify(trafficData).slice(0, 1000)}\n` +
      `\nSummary:`;
    const requestBody = {
      contents: [
        {
    ...
    const data = await response.json();
    // Extract the first candidate’s text. Gemini responses often nest content in
    // candidates[0].content.parts[0].text. Fallback to empty string if not
    // present.
    const summary =
    ...
    console.error(err);
    return 'Unable to generate traffic summary.';
  }
}
...
export const MunicipalProjectSystem: React.FC<MunicipalProjectSystemProps> = ({ className }) => {
  ...
  const [trafficSummary, setTrafficSummary] = useState<string>('');
  const [trafficLoading, setTrafficLoading] = useState<boolean>(false);
  // Cache traffic summaries by project ID to avoid redundant API calls.
  const trafficCache = useRef<Record<string, string>>({});
  // Load all projects on mount.
  useEffect(() => {
    loadProjects();
  }, []);
  // Recalculate conflicts whenever projects change.
  useEffect(() => {
    const conflicts = MunicipalProjectService.findConflictingProjects();
    setConflictingProjects(conflicts);
  }, [projects]);
  // Fetch traffic summary whenever a project is selected.
  useEffect(() => {
    if (!selectedProject) {
      setTrafficSummary('');
      return;
    ...
    if (trafficCache.current[projectId]) {
      setTrafficSummary(trafficCache.current[projectId]);
      return;
    }
    // Ensure the project has at least one point or polygon coordinate.
    const loc = selectedProject.locations?.[0];
    if (!loc) {
      setTrafficSummary('No location data available.');
      return;
    }
    // Derive a coordinate from the first location. For polygons, take the first
    // coordinate in the path. This could be improved by computing centroid.
    let coord: Coordinate | null = null;
    const point = parseGeometry(loc.location);
    if (point) {
      coord = point;
    } else {
      const poly = parsePolygonGeometry(loc.location);
      coord = poly && poly.length > 0 ? poly[0] : null;
    }
    if (!coord) {
      setTrafficSummary('No coordinate data available.');
      return;
    }
    // Fetch traffic info and then generate summary.
    (async () => {
      setTrafficLoading(true);
      const trafficData = await fetchTrafficInfo(coord);
      let summary = '';
      if (!trafficData) {
        summary = 'Traffic information could not be retrieved.';
      } else {
        summary = await generateTrafficSummary(selectedProject, trafficData);
      }
      trafficCache.current[projectId] = summary;
    ...
    <div className={`h-full flex flex-col ${className || ''}`}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-white">
        <div className="flex items-center gap-4">
          <h2 className="text-xl font-semibold text-gray-900">Municipal Project Management</h2>
          {/* Quick Stats */}
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => setView('map')}
            className={`px-3 py-2 text-sm rounded ${view === 'map' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:text-gray-900'}`}
          >
            <MapPin size={16} className="inline mr-1" /> Map View
          </button>
          <button
            onClick={() => setView('list')}
            className={`px-3 py-2 text-sm rounded ${view === 'list' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:text-gray-900'}`}
          >
            <Users size={16} className="inline mr-1" /> List View
          </button>
          <button
      ...
      {conflictCount > 0 && (
        <div className="bg-red-50 border-b border-red-200 px-4 py-2">
          <div className="flex items-center gap-2 text-red-700 text-sm">
            <AlertCircle size={16} />
            <span>
              {conflictCount} project{conflictCount > 1 ? 's have' : ' has'} scheduling or location conflicts. Review project timelines and
              locations to resolve overlaps.
            </span>
          </div>
        </div>
      )}
            ...
            <div className="p-4 border-b border-gray-200">
              ...
              {conflictingProjects.includes(selectedProject.id) && (
                <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded text-sm text-red-700">
                  <AlertCircle size={14} className="inline mr-1" /> This project has scheduling or location conflicts
                </div>
              )}
            </div>
            <div className="p-4">
              ...
              <div className="mt-6 pt-4 border-t border-gray-200">
                <h4 className="text-sm font-medium text-gray-700 mb-3">Traffic Impact</h4>
                {trafficLoading ? (
                  ...
                  <p className="text-sm text-gray-600 whitespace-pre-line">{trafficSummary || 'No analysis available.'}</p>
                )}
              </div>
              {/* Quick Actions */}
              <div className="mt-6 pt-4 border-t border-gray-200">
